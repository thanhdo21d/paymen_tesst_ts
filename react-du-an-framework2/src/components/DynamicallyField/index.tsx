...
const DynamicField = ({ submit, setSubmit, setDynamic, dataSize }: Props) => {
  ...
  useEffect(() => {
    field.forEach((item) => {
      (item.price as any) == ''
        ? (item.errors.price = 'Price size is required') && setSubmit(false)
        : '';
        ...
        {field.map((item, index) => (
          <div className="flex items-center gap-1" key={index}>
            <div className="mb-[15px]">
              ...
              />
              <span className="text-red-500 text-sm block h-[10px]">
